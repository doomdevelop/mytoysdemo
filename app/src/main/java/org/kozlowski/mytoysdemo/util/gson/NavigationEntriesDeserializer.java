...
public class NavigationEntriesDeserializer implements JsonDeserializer<Children> {
    ...
    public Children deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
        ...
        childrenNode.setChildren(iterateJsonArray(jsonArray,childrenNode));
        return childrenNode;
    }

    private List<Children> iterateJsonArray(JsonArray jsonArray, Children children){
    ...
    private Children parseChildren(JsonObject jsonObject){
        ...
        if(children instanceof ChildrenLink){
            ((ChildrenLink)children).setUrl(labelElement.getAsString());
        }else if(children instanceof ChildrenNode){
            ((ChildrenNode)children).setChildren(iterateJsonArray(childrenArray,children));
        }
        return children;
    }
}
